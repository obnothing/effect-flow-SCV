...
DATASET_ORDER = None  # Will be set from config
TRANSDUCTIVE_WARNING = (
    "The base encoder was initialized from a pretrained EVM-BERT. "
    "Stage 16B uses effect-flow chunks from configured datasets."
)
...
def prepare_data(config, distributed, rank, seed):
    ...
    for dataset_index, name in enumerate(DATASET_ORDER):
        ...
        holdout_seed = seed + dataset_index * 100003
        holdout_policy = str(config.get("internal_valid_split_policy", "contract_id"))
        if holdout_policy == "contract_id":
            train_offsets, valid_offsets = choose_internal_holdout_by_contract(
                path, offsets, ratio, holdout_seed
            )
        elif holdout_policy == "chunk":
            train_offsets, valid_offsets = choose_internal_holdout(
                offsets, ratio, holdout_seed
            )
        else:
            raise ValueError("internal_valid_split_policy must be contract_id or chunk")
        if rank == 0:
        ...
        source_states[name] = {
            ...
            "internal_valid_split_policy": holdout_policy,
    ...
    if rank == 0:
        ...
        data_state = {
            ...
            "global_vulnerability_label_names": stats_by_dataset[DATASET_ORDER[0]][
...
def main():
    ...
    DATASET_ORDER = list(config.get("train_corpora", {}).keys())
    ...
    main_print(
        rank,
        "[INFO] train chunks: " + ", ".join(
            f"{name}={len(source['offsets'])}"
            for name, source in zip(DATASET_ORDER, train_sources)
        ),
    )
    main_print(
        rank,
        "[INFO] internal valid: " + ", ".join(
            f"{name}={len(source['offsets'])}"
            for name, source in zip(DATASET_ORDER, valid_sources)
        ),
    )
        ...
        for micro_step, batch in enumerate(progress, start=1):
            ...
            with sync_context:
                ...
                if not torch.isfinite(loss):
                    print(f"[DEBUG] Non-finite loss at micro-step {micro_step}")
                    print(f"[DEBUG] total={loss.item()}")
                    for loss_key in ['mom_loss', 'etp_loss', 'efpp_loss', 'err_loss', 'vep_loss', 'vtm_loss']:
                        val = outputs[loss_key]
                        print(f"[DEBUG] {loss_key}={val.item()}, is_finite={torch.isfinite(val).item()}")
            ...
            print(
                ...
                "sources=" + ",".join(
                    ...
                    for name, count in zip(
                        DATASET_ORDER,
                        (row["bjut_samples"], row["dive_samples"]),
                    )
                ),
                flush=True,
            )
    ...
    if rank == 0:
        ...
        total_by_dataset = dict(zip(DATASET_ORDER, (total_bjut, total_dive)))
        losses_valid = all(
        ...
        report = {
            ...
            "internal_valid_split_policy": str(
                config.get("internal_valid_split_policy", "contract_id")
            ),
            ...
            "actual_sample_counts": {
                name: {
                    "samples": count,
                    "ratio": count / total_seen,
                }
                for name, count in total_by_dataset.items()
            },
            "efpp_pattern_subset": "conservative_22",
