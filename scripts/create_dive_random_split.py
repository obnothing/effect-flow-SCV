...
def parse_args():
    parser = argparse.ArgumentParser(description="Create non-grouped random DIVE split.")
    parser.add_argument("--source_data_dir", default="data/processed/DIVE")
    parser.add_argument("--output_data_dir", default="data/processed/DIVE_random_split")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--train_ratio", type=float, default=0.8)
    parser.add_argument("--valid_ratio", type=float, default=0.1)
    parser.add_argument("--test_ratio", type=float, default=0.1)
    parser.add_argument("--report_path", default="data/reports/dive_random_split_report.txt")
...
def write_report(report, txt_path):
    ...
    lines = ["DIVE random split report", ""]
...
def main():
    ...
    for split, path in paths.items():
        if not path.exists():
            raise FileNotFoundError(f"Missing DIVE strict split: {path}")
        ...
        for output_split, split_refs in assignments.items():
            ...
            with (output_dir / f"{output_split}.jsonl").open("w", encoding="utf-8") as out:
                for source_split, offset in split_refs:
                    ...
                    if len(labels) != len(DIVE_LABEL_NAMES):
                        raise ValueError(f"Expected 8 DIVE labels, got {len(labels)}")
                    clean = {
                        "id": str(row["id"]),
                    ...
                    ids.add(clean["id"])
            label_counts[output_split] = dict(zip(DIVE_LABEL_NAMES, counts))
    ...
    mapping = {
        ...
        "label_to_id": {name: idx for idx, name in enumerate(DIVE_LABEL_NAMES)},
        "id_to_label": {str(idx): name for idx, name in enumerate(DIVE_LABEL_NAMES)},
        "source": "DIVE_Labels.csv",
    }
    ...
    for split in ["train", "valid", "test"]:
        ...
        pretrain_exposure[split] = {
            ...
            "ids_seen_in_dive_strict_train_mlm": id_seen,
            "id_seen_ratio": id_seen / max(1, sample_count),
            "opcode_hashes_seen_in_dive_strict_train_mlm": hash_seen,
    ...
    report = {
        "status": "warning",
        "warning": (
            "This is a non-grouped random split. The continued EVM-BERT was pretrained "
            "on the previous DIVE strict-train opcode inputs, so random valid/test may "
            "contain MLM-seen contracts. This is not strict generalization."
        ),
        ...
        "num_labels": len(DIVE_LABEL_NAMES),
        "label_names": DIVE_LABEL_NAMES,
        ...
        "leakage_status": "warning" if any(overlaps.values()) else "ok",
        "continued_mlm_exposure": pretrain_exposure,
        "continued_mlm_setting": "transductive_under_this_random_split",
    }
