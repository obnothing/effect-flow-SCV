...
def metrics_from_predictions(predictions):
    ...
    return {
        ...
        "dos": per_label[DOS_ID],
        "front_fp_confounders": front_fp_confounders,
    }


def valid_per_label_rows(eval_dir):
    data = load_json(eval_dir / "per_label_thresholds_valid.json")
    if data and data.get("per_label"):
        return data["per_label"]
    return []
...
def collect_eval_row(item, checkpoint_tag, mode, filename):
    ...
    row = {
        "status": "ok",
        "variant": item["variant"],
        "graph_scale": item.get("graph_scale"),
        "checkpoint_tag": checkpoint_tag,
        "threshold_mode": mode,
        ...
        "dos_tp": metrics["dos"]["tp"],
        "dos_fp": metrics["dos"]["fp"],
        "dos_fn": metrics["dos"]["fn"],
        "rare_mean_f1": (
        ...
        "dos_delta_vs_baseline": metrics["dos"]["f1"] - BASELINE["dos_f1"],
        "graph_evidence_enabled": train_summary.get("graph_evidence_enabled"),
        "graph_evidence_scale": train_summary.get("graph_evidence_scale"),
        "graph_evidence_attention_scale": train_summary.get(
            "graph_evidence_attention_scale"
        ),
        "graph_evidence_logit_scale": train_summary.get("graph_evidence_logit_scale"),
    }
    return row
...
def write_outputs(rows, output_prefix):
    ...
    lines = [
        "DIVE graph evidence search summary",
        "",
        "baseline:",
    ...
    for row in ok_rows:
        lines.append(
            ...
            f"{row['dos_f1']:.6f} | {row['rare_mean_f1']:.6f} | "
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Summarize DIVE graph evidence search."
    )
    parser.add_argument(
        "--manifest",
        default="configs/generated/dive_graph_evidence_search/manifest.yaml",
    )
    parser.add_argument(
        "--output_prefix",
        default="results/dive_graph_evidence_search/summary",
    )
