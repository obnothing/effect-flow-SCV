...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Summarize DIVE major-label v2 single-task search."
    )
    parser.add_argument(
        "--manifest",
        default="configs/generated/dive_major_label_v2_single_task/manifest.yaml",
    )
    parser.add_argument(
        "--result_root",
        default="results/dive_major_label_v2_single_task",
    )
    parser.add_argument(
        "--output_prefix",
        default="results/dive_major_label_v2_single_task/summary",
    )
    parser.add_argument(
        "--checkpoints",
        nargs="*",
...
def main():
    ...
    lines = ["DIVE major-label v2 single-task summary", ""]
